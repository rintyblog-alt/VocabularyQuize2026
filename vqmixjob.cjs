/* 文字 PDF・混在 PDF をプリセット経路で測る（ケース 1-C）。
...
(async () => {
  ...
  /* 混在の確認: 文字 PDF に加えて、画像ページも 2 枚足す。
  ...
  if (process.env.VQ_MIX === "1") {
    ...
    for (const f of imgs) {
      i++;
      const u = await upload(f, jobId, { kind: "image", mimeType: "image/png" });
      attachments.push({
        id: u.id, attachmentId: u.id, jobId, name: "画像資料（" + i + "ページ）.png",
        kind: "image", fileType: "image", mimeType: "image/png", bytes: u.bytes,
        pageNumber: i
      });
    }
    console.log("画像ページ " + imgs.length + " 枚を足しました（混在）");
  }
  ...
  const body = JSON.stringify({
    ...
    options: { questionCount: COUNT, sourceOnly: true, structuredOutput: "preset",
               questionTypes: new Array(COUNT).fill("multiple_choice"), choiceCount: 5 },
    stream: true
  });
