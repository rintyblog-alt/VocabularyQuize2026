/* ══════════════════════════════════════════════════════════════════════
   ...
   ★ 触るときの注意
     ...
     ・normalize が **情報を捨てる**のは 3 か所だけ。呼ぶ側はここを知っておくこと:
         1) MAX_TRACKS を超えたトラック（engine が持てない本数は残しても直せない）
         2) 素材を指せていない video/image/audio クリップ → 灰色の図形（§1-5）。
            ＝ メディアの clip は **必ず assetId 付きで**作る（asset.add が先）。
         3) 契約書に無いフィールド（組み立て直すので落ちる）
