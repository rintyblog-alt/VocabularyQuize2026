/* プリセット経路（資料作問）を測る。画面と同じ要求（structuredOutput: "preset"）を送る。
...
(async () => {
  ...
  const body = JSON.stringify({
    message: "この資料の全ページから " + COUNT + " 問つくってください。5 択で、解説は詳しくしてください。",
    attachments,
    /* 画面の「資料作問」と同じ形。5 択・詳しい解説を明示する。 */
    taskHint: "preset_generation",
    options: { questionCount: COUNT, sourceOnly: true, structuredOutput: "preset",
               questionTypes: new Array(COUNT).fill("multiple_choice"),
               choiceCount: 5 },
    stream: true
  });
  ...
  const events = [];
  const stats = { maxGapMs: 0, heartbeats: 0 };
  await new Promise((resolve, reject) => {
    ...
    const req = http.request({
      ...
      let buf = "", ev = null, last = 0, maxGap = 0, heartbeats = 0;
      res.setEncoding("utf8");
      res.on("data", (chunk) => {
        const nowMs = hr();
        if (last) maxGap = Math.max(maxGap, nowMs - last);
        last = nowMs;
        stats.maxGapMs = maxGap; stats.heartbeats = heartbeats;
        buf += chunk;
        ...
        for (const line of lines) {
          if (line.startsWith("event:")) ev = line.slice(6).trim();
            ...
            if (ev === "heartbeat") { heartbeats++; stats.heartbeats = heartbeats; continue; }
  ...
  console.log("\n総時間 " + (totalMs / 1000).toFixed(1) + " 秒 ／ SSE " + events.length + " 件"
    + " ／ heartbeat " + stats.heartbeats + " 回 ／ 無音の最長 "
    + (stats.maxGapMs / 1000).toFixed(1) + " 秒\n");
  ...
  fs.writeFileSync(out, JSON.stringify({ totalMs, events, stats }, null, 2));
  console.log("記録:", out);
})();
