  ...
  var LIMITS = {
    ...
    maxBytesByKind: {
      ...
      /* ★ 2026-09-03・訴え「PDF、スキャン画像は 100MB まで 読み取れるように」。
         スキャンした 1 枚の 写真は 20MB を 超える ことが ある（A3 を 600dpi）。 */
      image: 100 * 1024 * 1024,            /* 画像 100MB */
    ...
    /* ★ 2026-09-03・訴え「PDF・スキャン画像から 均等に 出題されない」。
       ここが 12 だったので、**何ページ ある PDF でも 先頭 12 ページしか
       読んでいなかった**。だから 何回 作らせても 同じ ところが 出る。
       読み取りは 8 ページずつ・6 本 同時で 回すので、60 でも 2 巡で 済む。 */
    maxScanPages: 60,
    /* ★ 読み取り（OCR）に 回せる 大きさも 100MB へ（2026-09-03・訴え）。
       ★ ただし これは **画面の 中に 抱える 絵の 合計**なので、
         小さい 端末では 落ちる。持ち物の 少ない 端末は 32MB で 止める
         （1 ページ 0.5〜1.2MB なので、それでも 30 ページ 以上 読める）。 */
    get maxScanBytes() {
      var m = 0;
      try { m = Number(navigator.deviceMemory) || 0; } catch (e) { m = 0; }
      return (m && m < 6) ? 32 * 1024 * 1024 : 100 * 1024 * 1024;
    },
    /* 1ファイルから取り込む本文の上限。
