...
class VqMicDown extends AudioWorkletProcessor {
  constructor(opts) {
    ...
    this.at = 0;
    this.muted = false;
    /* ══ 止めている間の 声を **少しだけ 覚えておく**（2026-08-19）════════
       ★ 訴え「Lumi が返答中に 指示しても通らず、2 度言ってやっと通る」。
         原因は はっきりしている。Lumi が話している間、こちらの声は
         **1 かたまりも 送っていなかった**。割り込みに気づいて マイクを
         開けた頃には、言葉の **頭が すでに 終わっている**。
         だから 1 回目は 半分しか届かず、意味が通らなかった。
       ★ 直しかた: 止めている間も 変換だけは 続けて、**直近 1.2 秒**を
         輪っかの入れ物に 残す。割り込みだと 分かった瞬間に、
         その 1.2 秒を 先に 送る。言葉の 頭から 届く。
       ★ 送らない限り 外へは 出ない（残すだけ）。回り込みにはならない。 */
    this.keep = Math.round(this.target * ((o.keepMs || 1200) / 1000));
    this.ring = new Int16Array(this.keep);
    this.rat = 0;     /* 次に書く場所 */
    this.rn = 0;      /* 溜まっている数 */
    this.mpos = 0;    /* 止めている間の 間引きの位置 */
    this.port.onmessage = function (e) {
      var d = e && e.data;
      if (!d) return;
      if (d.type === "mute") this.muted = !!d.on;
      else if (d.type === "clearkeep") { this.rn = 0; this.rat = 0; this.mpos = 0; }
      else if (d.type === "flush") {
        var n = this.rn, L = this.ring.length;
        var out = new Int16Array(n);
        var s0 = ((this.rat - n) % L + L) % L;
        for (var k = 0; k < n; k++) out[k] = this.ring[(s0 + k) % L];
        this.rn = 0; this.rat = 0; this.mpos = 0;
        this.port.postMessage({ type: "preroll", ms: Math.round(n / this.target * 1000),
                                buf: out.buffer }, [out.buffer]);
      }
    }.bind(this);
  }
  process(inputs) {
    ...
    if (this.muted) {
      /* 送らない。**覚えるだけ。** 溢れたら 古いものから 上書きする。 */
      var L2 = this.ring.length;
      for (var mi = 0; mi < ch.length; mi++) {
        this.mpos += 1;
        if (this.mpos < this.ratio) continue;
        this.mpos -= this.ratio;
        var mv = ch[mi];
        if (mv > 1) mv = 1; else if (mv < -1) mv = -1;
        this.ring[this.rat] = mv < 0 ? mv * 0x8000 : mv * 0x7FFF;
        this.rat = (this.rat + 1) % L2;
        if (this.rn < L2) this.rn += 1;
      }
