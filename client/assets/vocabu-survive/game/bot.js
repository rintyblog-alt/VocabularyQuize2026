...
export class Bot {
  ...
  findLanding(dirx, dirz, prog, tNow, 縁まで) {
    ...
    /* ★ 着いた あと すぐ 消える 所は **強く 減点する**。
       着いた 瞬間だけ 見ると、0.16 秒 後に 消える 板へ 跳んで 落ちる
       （c17 で 実測）。 */
      ...
      for (let k = 0; k < 11; k++) {
        ...
        if (score > bestScore - 4) {
          ...
          if (先も見る) {
            ...
            let 後ok = 後で.get(key);
            if (後ok === undefined) {
              /* ★ どれくらい 先まで 残っていて ほしいか。
                 1.1 秒 に すると 慎重すぎて 待ちすぎ、通過が 27→26 に 落ちた。
                 0.55 秒（着いて すぐ 跳び直せる 分）が ちょうど よい。 */
            ...
            /* ★ **強く 減点する が、外しはしない。**
               「安全な ものが 1 つでも あれば そちらだけ」に すると
               消える板の コースで 待ちすぎて 通過が 27→26 に 落ちた（実測）。
               減点なら、ほかに ましな ものが あれば そちらへ 行く。 */
            if (!後ok) score -= 4.5;   /* 減点の 強さは 3.0/4.5/6.0 と 余裕 0.35〜1.1 を 総当たりして 決めた */
          }
