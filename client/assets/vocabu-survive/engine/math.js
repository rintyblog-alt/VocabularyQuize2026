/* ══════════════════════════════════════════════════════════════════════════
   小さな 数の道具。行列は 列優先（WebGL と 同じ並び）。

   置き換え（out）を 必ず 受け取る形にしてある。**毎フレーム 新しい配列を
   作らない**ため。1 試合で 数百万回 呼ぶので、ここでの ごみが そのまま
   カクつきに なる。
   ══════════════════════════════════════════════════════════════════════════ */
...
export function smoothstep(a, b, x) { const t = clamp((x - a) / (b - a || 1e-6), 0, 1); return t * t * (3 - 2 * t); }
/** 角の 差を -π〜π に 畳む */
export function angleDelta(a, b) { let d = (b - a) % TAU; if (d > Math.PI) d -= TAU; if (d < -Math.PI) d += TAU; return d; }
/** 時間の 長さに 依らない 追従（dt が 変わっても 同じ 速さで 寄る） */
export function damp(a, b, lambda, dt) { return lerp(a, b, 1 - Math.exp(-lambda * dt)); }
export function dampAngle(a, b, lambda, dt) { return a + angleDelta(a, b) * (1 - Math.exp(-lambda * dt)); }
...
export const m4 = {
  ...
  /** 位置・Y回り・大きさ から 直接 作る（毎フレーム 使うので 掛け算を 展開してある） */
  compose(o, px, py, pz, ry, sx, sy, sz) {
  ...
  /** 3 軸の 回りを 使う版（仕掛けの 羽根・振り子 用） */
  composeXYZ(o, px, py, pz, rx, ry, rz, sx, sy, sz) {
...
/* ── 決まった 乱数（種から 同じ列を 作る）──────────────────────────────
   コースの 飾りを 「毎回 同じ」にするために 要る。
   Math.random だと 検査の たびに 形が 変わって 測れない。 */
