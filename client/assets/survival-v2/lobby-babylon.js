...
export class SurvivalLobbyBabylon {
  ...
  _tick() {
    ...
    for (const entry of this.avatarNodes.values()) {
      ...
      if (entry.ring) {
        const pulse = 1 + Math.sin(now * 2.1 + entry.seed) * (entry.isMe ? 0.05 : 0.03);
        entry.ring.scaling.x = pulse;
        entry.ring.scaling.z = pulse;
      }
    }
  }
  ...
  _upsertAvatar(player, targetPosition, isMe) {
    ...
    if (!entry) {
      const root = new BABYLON.TransformNode(`avatar-${id}`, this.scene);
      const bodyMat = new BABYLON.StandardMaterial(`avatar-body-${id}`, this.scene);
      const palette = ["#ff6fa5", "#6fc4ff", "#ffd16a", "#7be39b", "#b48eff", "#ff9e6f"];
      bodyMat.diffuseColor = color3(palette[stableIndexFromId(id, palette.length)]);
      bodyMat.emissiveColor = color3("#05070c");

      const body = BABYLON.MeshBuilder.CreateCapsule(`avatar-bodyMesh-${id}`, {
        height: 1.75,
        radius: 0.42,
        tessellation: 10
      }, this.scene);
      ...
      const face = BABYLON.MeshBuilder.CreatePlane(`avatar-face-${id}`, { width: 0.44, height: 0.54 }, this.scene);
      const faceMat = new BABYLON.StandardMaterial(`avatar-face-mat-${id}`, this.scene);
      faceMat.diffuseColor = color3("#f7fbff");
      faceMat.emissiveColor = color3("#ffffff").scale(0.08);
      face.material = faceMat;
      face.parent = root;
      face.position.y = 1.16;
      face.position.z = 0.36;

      const ring = BABYLON.MeshBuilder.CreateTorus(`avatar-ring-${id}`, {
        diameter: isMe ? 2.35 : 1.9,
        thickness: 0.06,
        tessellation: 40
      }, this.scene);
      const ringMat = new BABYLON.StandardMaterial(`avatar-ring-mat-${id}`, this.scene);
      ringMat.emissiveColor = color3(isMe ? "#79b7ff" : "#5fd6a6");
      ringMat.alpha = 0.92;
      ring.material = ringMat;
      ring.parent = root;
      ring.rotation.x = Math.PI / 2;
